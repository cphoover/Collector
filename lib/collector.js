...
function Collector(_config){
	this.options = _.defaults({
		...
		"noisy"    : true,
		"tail_cmd" : "tail -F --lines=0 %s"//this stuff should be in an options part of the config
	}, _config);
...
Collector.prototype.init = function(){
	// @todo iterate over top level of config and create objects keyed to name...
	// omitting remotes to send to.. these could be designated via input objects
	// which have a stream field....
	// e.g TailInput, ExecInput, MongoInput in a plugins directory
	// e.g. files, exec, mongo
	/*
		this.inputs = _(this.config)
			.omit('remotes') //or put in sub object
			.map(function(_inputSettings, _inputName){
				var stream =  new inputMap[_inputName](_inputSettings).stream;
				stream.pipe(this.udpStream);
				return stream;
			}).value()
	*/
...
Collector.prototype.tail = function(filename){
	var str = hi(exec(format(this.options.tail_cmd, filename)).stdout);
	return str;
//	return exec(format(this.config.tail_cmd, filename)).stdout;
};
