...
//		readStream.pipe(this.udpStream);
		...
		if(this.options.noisy) { readStream.fork().pipe(process.stdout); }
		return readStream;
...
Collector.prototype.wrapStreamWithKeyAndHost = function(readStream, key) {
	...
	var newStream = hi(readStream).split().map(function(dat){
		var str = fmt("[%s] [%s] - %s", hostname, key, dat) + "\n";
		return this.options.type && fmt("[%s] ", this.options.type) + str || str;
	}.bind(this));
...
Collector.prototype.tail = function(filename){
	var str = hi(exec(fmt(this.options.tailCmd, filename)).stdout);
	return str;
};
